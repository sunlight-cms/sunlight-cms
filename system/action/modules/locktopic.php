...
$query = DB::query("SELECT p.id,p.time,p.subject,p.locked,r.slug forum_slug," . $userQuery['column_list'] . " FROM " . _posts_table . " p JOIN " . _root_table . " r ON(p.home=r.id) " . $userQuery['joins'] . " WHERE p.id=" . $id . " AND p.type=5 AND p.xhome=-1");
if (DB::size($query) != 0) {
    $query = DB::row($query);
...
if (isset($_POST['doit'])) {
    DB::query("UPDATE " . _posts_table . " SET locked=" . (($query['locked'] == 1) ? 0 : 1) . " WHERE id=" . $id);
