...
$query = DB::queryRow("SELECT p.id,p.time,p.subject,p.locked,r.slug forum_slug,r.layout forum_layout," . $userQuery['column_list'] . " FROM " . _post_table . " p JOIN " . _page_table . " r ON(p.home=r.id) " . $userQuery['joins'] . " WHERE p.id=" . $id . " AND p.type=" . Post::FORUM_TOPIC . " AND p.xhome=-1");
if ($query !== false) {
...
if (isset($_POST['doit'])) {
    DB::update(_post_table, 'id=' . DB::val($id), ['locked' => (($query['locked'] == 1) ? 0 : 1)]);
