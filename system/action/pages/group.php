    ...
    while ($item = DB::row($items)) {
        ...
        if ($_page['var1'] == 1) {
            ...
            switch ($item['type']) {
                ...
                case Page::SECTION:
                    if ($item['var1'] == 1) {
                        $iteminfos['comment_num'] = [_lang('article.comments'), DB::count(_post_table, 'type=' . Post::SECTION_COMMENT . ' AND home=' . DB::val($item['id']))];
                    }
                    break;
                ...
                case Page::BOOK:
                    ...
                    $lastpost = DB::queryRow("SELECT p.author,p.guest," . $userQuery['column_list'] . " FROM " . _post_table . " p " . $userQuery['joins'] . " WHERE p.home=" . $item['id'] . " ORDER BY p.id DESC LIMIT 1");
                    if ($lastpost !== false) {
                    ...
                    $iteminfos['post_num'] = [_lang('global.postsnum'), DB::count(_post_table, 'type=' . Post::BOOK_ENTRY . ' AND home=' . DB::val($item['id']))];
                ...
                case Page::GALLERY:
                    ...
                    // forum
                case Page::FORUM:
                    $iteminfos['topic_num'] = [_lang('global.topicsnum'), DB::count(_post_table, 'type=' . Post::FORUM_TOPIC . ' AND home=' . DB::val($item['id']) . ' AND xhome=-1')];
                    $iteminfos['answer_num'] = [_lang('global.answersnum'), DB::count(_post_table, 'type=' . Post::FORUM_TOPIC . ' AND home=' . DB::val($item['id']) . ' AND xhome!=-1')];
                    break;
