    ...
    static function init(): array
    {
        ...
        if (Settings::get('captcha') && !User::isLoggedIn()) {
            ...
            return [
                ...
                'content' => '<input type="text" name="_cp" class="inputc" autocomplete="off"><img src="' . _e(Router::path('system/script/captcha/image.php', ['query' => ['n' => $captchaCounter]])) . '" alt="captcha" title="' . _lang('captcha.help') . '" class="cimage"><input type="hidden" name="_cn" value="' . $captchaCounte . '">' ,
                'top' => true,
                'class' => 'captcha-row',
            ];
        }
